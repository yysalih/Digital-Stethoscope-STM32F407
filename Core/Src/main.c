...
int32_t raw_val;
int32_t centered;
int32_t current_center;
uint16_t processed_samples[128];
uint16_t heart_buffer[1000];
float filtered_value = 1800.0f;
float alpha = 0.01f;
volatile uint8_t data_ready = 0;
volatile int16_t current_gain = 2;

void Process_Heartbeat(uint16_t raw_sample) {
    // Basic Low-Pass Filter logic
    filtered_value = (alpha * raw_sample) + ((1.0f - alpha) * filtered_value);
}
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
    if (GPIO_Pin == GPIO_PIN_0) {
    	if (current_gain == 2) current_gain = 4;
    	else if (current_gain == 4) current_gain = 8;
    	else if (current_gain == 8) current_gain = 1;
    	else current_gain = 2;
    }
}
...
int main(void)
{
  ...
  HAL_ADC_Start_DMA(&hadc1, (uint32_t*)heart_buffer, 1000);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  ...
	  for(int x = 0; x < 128; x++) {
	  	raw_val = processed_samples[x];

	  	centered = (int32_t)raw_val - current_center;

	  	int16_t y_plot = 32 - (centered * current_gain);

	  	if(y_plot < 0)  y_plot = 0;
	  	if(y_plot > 63) y_plot = 63;
	  ...
	  ssd1306_SetCursor(95, 0); // top right
	  ssd1306_WriteString(zoom_str, Font_7x10, 1);
